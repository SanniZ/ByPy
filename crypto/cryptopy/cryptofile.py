...
from . import cryptoutils, cryptokey
...
def encrypto_path_files(key, src, dst, ftype, xtype=None, cipher='AES', mode='CBC'):
    ...
    if fs:
        enc = cryptoutils.CryptoUtils(key, cipher=cipher, mode=mode)
        res = []
        for k, v in fs.items():
...
def decrypto_path_files(key, src, dst, ftype, xtype=None, cipher='AES', mode='CBC'):
    fs = get_type_files(src, ftype)
    if fs:
        dec = cryptoutils.CryptoUtils(key=key, cipher=cipher, mode=mode)
        res = []
        for k, v in fs.items():
            with open(os.path.join(v, k), 'r', encoding='UTF-8') as fd:
                f = fd.read()
...
class CryptoFile(object):
    ...
    def encrypto_files(self):
        return encrypto_path_files(self._key, self._src, self._dst,
                        self._ftype, self._xtype, self._cipher, self._mode)

    def decrypto_files(self):
        return decrypto_path_files(self._key, self._src, self._dst,
                        self._ftype, self._xtype, self._cipher, self._mode)
...
class CryptoFileTypeCmds(object):
    ...
    def main(self, args=None):
        ...
        if not key:
            ...
            key = cryptokey.CryptoKey().get_key_from_file(key_file_path)
            if not key:
                print('No found key!!!')
                return False
        # run opts.
